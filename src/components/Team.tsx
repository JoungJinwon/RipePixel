export default function Team() {
  ...
  return (
    <section id="team" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className={`grid gap-8 ${team.length === 1 ? 'justify-center' : 'md:grid-cols-2 lg:grid-cols-4'}`}>
        {team.map((member, index) => (
          <div
            key={index}
            className={`group bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-purple-500/20 hover:border-purple-500/40 transition-all duration-300 hover:transform hover:scale-105 text-center ${team.length === 1 ? 'max-w-sm' : ''}`}
          >
